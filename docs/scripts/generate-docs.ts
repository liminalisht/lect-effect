...
const resolveFromRoot = (relativePath: string) => path.join(repoRoot, relativePath);
const docgenBin = resolveFromRoot('docs/node_modules/.bin/docgen');

const docgenTasks: readonly DocgenTask[] = [
  {
    name: 'backend',
    cwd: 'apps/backend',
...
const runDocgenTask = (task: DocgenTask) =>
  Effect.gen(function * () {
    ...
    const {stdout, stderr} = yield * Effect.tryPromise(async () => execFileAsync(
      docgenBin,
      buildDocgenArgs(task),
      {
        cwd: resolveFromRoot(task.cwd),
        env: process.env,
        encoding: 'utf8',
      },
    ));
...
const moveModuleIndex = (section: string) =>
  Effect.gen(function * () {
