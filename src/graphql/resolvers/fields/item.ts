/**
 * Field resolvers for the Item type.
 * @since 1.0.0
 */
import { field, resolver } from '@gqloom/core';
import { Schema } from 'effect';
import { runEffect } from '../../effect';
import { itemSchema, type Item } from '../../../domain/item/item';
import { productSchema } from '../../../domain/product/product';
import { productForItem } from '../../../handlers/item';

const productForItemField = field(Schema.standardSchemaV1(Schema.NullOr(productSchema))) // todo: extract proper schema w/ NullOr
  .description('Fetch the product associated with the item.')
  .resolve(async (parent: Item) => runEffect(productForItem(parent.id)));
/**
 * Resolver map for Item fields.
 * @since 1.0.0
 */
export const itemFieldResolvers = resolver.of(
  Schema.standardSchemaV1(itemSchema),
  {
    productForItem: productForItemField,
  },
);
